...
## function 
draw_map <- function(bb, zoom=9, scale_location="br") {
  ...
  source_map <- ggplot() +
    ...
    ggspatial::annotation_scale(location = scale_location, height = unit(0.1, "cm"))
...
draw_map(bbE)
ggsave(dpi = 300, filename = "Enisey2.svg")


#Angara Irkutsk
bbA <- c(left = 104.2, bottom = 52.2, right = 104.4, top = 52.3)
draw_map(bbA, zoom=12)
...
no#devtools::install_github("liamgilbey/ggwaffle")
library(ggwaffle)
